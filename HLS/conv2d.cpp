...
void rowCaching(TFXP *input, TFXP rows_buffer[CONV_SIZE][MAX_ROW_SIZE], uint32_t inputWidth, uint32_t y) {
   ...
   line : for (uint32_t cx = 0; cx < inputWidth; ++cx)
    {
        #pragma HLS loop_tripcount max=256
        #pragma HLS PIPELINE II=1

        column : for (uint32_t cy = 0; cy < CONV_SIZE; ++cy) {
            #pragma HLS UNROLL
      ...
      uint32_t convWidth, uint32_t convHeight)
{
    ...
    #pragma HLS INTERFACE m_axi port=input offset=slave max_read_burst_length=256 bus=train1
    #pragma HLS INTERFACE m_axi port=output offset=slave max_write_burst_length=256 bus=train2
    #pragma HLS INTERFACE m_axi port=coeffs offset=slave max_read_burst_length=256 bus=train2
    ...
    #pragma HLS ARRAY_PARTITION variable=rows_buffer cyclic factor=3
